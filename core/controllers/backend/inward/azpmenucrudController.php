<?php
	...
	class azpmenucrudController extends Controllers {
		public function __construct() {
			...
			$var_m_azp_menu_crud = new m_azp_menu_crudModel;
			# Instanciando clases del modelo
				...
				switch ($var_get_method) {
					...
					case 'apupdate':
						...
						if (false != $var_m_zz_design->sql_permits_value_update($var_controller)) {
							...
							$var_file = fopen(ROUTE . CURRENT_PATH . 'api/http/dat_az.php', 'w') or die ('Error: No se creo ningun archivo');
							# Escritura de un archivo en modo binario seguro
							fwrite($var_file, '<?php' . "\n");
							# Consultando tablas de la base de datos
							$var_tables = $var_m_zz_design->sql_show_full_tables();
							# Recorriendo las tablas consultadas
							foreach ($var_tables as $tables):
								# Escritura de un archivo en modo binario seguro
								fwrite($var_file, '	' . "\n");
								fwrite($var_file, '	# Return devuelve un json con información acerca del éxito o posibles errores.' . "\n");
								fwrite($var_file, '	# Tabla: ' . $tables[0] . "\n");
								
								fwrite($var_file, '	$app->post("/' . $tables[0] . '/create", function($request, $response) {' . "\n");
								fwrite($var_file, '		$model = new m_' . $tables[0] . 'Model;' . "\n");
								fwrite($var_file, '		$response->withJson($model->sql_' . $tables[0] . '_insert($_POST));' . "\n");
								fwrite($var_file, '		return $response;' . "\n");
								fwrite($var_file, '	});' . "\n");
								
								fwrite($var_file, '	$app->post("/' . $tables[0] . '/delete", function($request, $response) {' . "\n");
								fwrite($var_file, '		$model = new m_' . $tables[0] . 'Model;' . "\n");
								fwrite($var_file, '		$response->withJson($model->sql_' . $tables[0] . '_delete($_POST));' . "\n");
								fwrite($var_file, '		return $response;' . "\n");
								fwrite($var_file, '	});' . "\n");
								
								fwrite($var_file, '	$app->post("/' . $tables[0] . '/file", function($request, $response) {' . "\n");
								fwrite($var_file, '		$model = new m_' . $tables[0] . 'Model;' . "\n");
								fwrite($var_file, '		$response->withJson($model->sql_' . $tables[0] . '_file($_POST));' . "\n");
								fwrite($var_file, '		return $response;' . "\n");
								fwrite($var_file, '	});' . "\n");
								
								fwrite($var_file, '	$app->post("/' . $tables[0] . '/remove", function($request, $response) {' . "\n");
								fwrite($var_file, '		$model = new m_' . $tables[0] . 'Model;' . "\n");
								fwrite($var_file, '		$response->withJson($model->sql_' . $tables[0] . '_remove($_POST));' . "\n");
								fwrite($var_file, '		return $response;' . "\n");
								fwrite($var_file, '	});' . "\n");
								
								fwrite($var_file, '	$app->post("/' . $tables[0] . '/restore", function($request, $response) {' . "\n");
								fwrite($var_file, '		$model = new m_' . $tables[0] . 'Model;' . "\n");
								fwrite($var_file, '		$response->withJson($model->sql_' . $tables[0] . '_restore($_POST));' . "\n");
								fwrite($var_file, '		return $response;' . "\n");
								fwrite($var_file, '	});' . "\n");
								
								fwrite($var_file, '	$app->post("/' . $tables[0] . '/state", function($request, $response) {' . "\n");
								fwrite($var_file, '		$model = new m_' . $tables[0] . 'Model;' . "\n");
								fwrite($var_file, '		$response->withJson($model->sql_' . $tables[0] . '_state($_POST));' . "\n");
								fwrite($var_file, '		return $response;' . "\n");
								fwrite($var_file, '	});' . "\n");
								
								fwrite($var_file, '	$app->post("/' . $tables[0] . '/table", function($request, $response) {' . "\n");
								fwrite($var_file, '		$model = new m_' . $tables[0] . 'Model;' . "\n");
								fwrite($var_file, '		$response->withJson($model->sql_' . $tables[0] . '_table($_POST));' . "\n");
								fwrite($var_file, '		return $response;' . "\n");
								fwrite($var_file, '	});' . "\n");
								
								fwrite($var_file, '	$app->post("/' . $tables[0] . '/update", function($request, $response) {' . "\n");
								fwrite($var_file, '		$model = new m_' . $tables[0] . 'Model;' . "\n");
								fwrite($var_file, '		$response->withJson($model->sql_' . $tables[0] . '_update($_POST));' . "\n");
								fwrite($var_file, '		return $response;' . "\n");
								fwrite($var_file, '	});' . "\n");
							endforeach;
							# Escritura de un archivo en modo binario seguro
							...
							# Crear archivo database.php en la ruta de core
							$var_file = fopen(ROUTE . CURRENT_PATH . 'core/database.php', 'w') or die ('Error: No se creo ningun archivo');
							# Escritura de un archivo en modo binario seguro
							fwrite($var_file, '<?php' . "\n");
							# Consultando tablas de la base de datos
							$var_tables = $var_m_zz_design->sql_show_full_tables();
							...
							foreach ($var_tables as $tables):
								# Escritura de un archivo en modo binario seguro
								fwrite($var_file, '	' . "\n");
								fwrite($var_file, '	# Tablas de la base de datos' . "\n");
								# Definiendo constantes de cada una de las tablas
								fwrite($var_file, "	define('" . $tables[0] . "', '" . $tables[0] . "');" . "\n");
								fwrite($var_file, "	define('t_" . $tables[0] . "', DATABASE['name'] . '.' . '" . $tables[0] . "');" . "\n\n");
								# Consultando columnas de cada una de las tablas
								$var_columns = $var_m_zz_design->sql_show_full_columns($tables[0]);
								fwrite($var_file, '	' . "\n");
								fwrite($var_file, '	# Campos de la tabla ' . $tables[0] . "\n");
								# Recorriendo las columnas consultadas
								foreach ($var_columns as $columns):
									# Definiendo constantes de cada una de las columnas
									fwrite($var_file, "	define('d_" . $tables[0] . '_' . $columns[0] . "', '" . $columns[0] . "');" . "\n");
								endforeach;
								fwrite($var_file, "\n");
							endforeach;
							# Escritura de un archivo en modo binario seguro
							...
							# Crear archivo baseurl.php en la ruta de core
							$var_file = fopen(ROUTE . CURRENT_PATH . 'core/baseurl.php', 'w') or die ('Error: No se creo ningun archivo');
							# Escritura de un archivo en modo binario seguro
							fwrite($var_file, '<?php' . "\n");
							# Consultando tablas de la base de datos
							$var_tables = $var_m_zz_design->sql_show_full_tables();
							...
							foreach ($var_tables as $tables):
								# Escritura de un archivo en modo binario seguro
								fwrite($var_file, '	# Url Reemplazo _ para la palabra completa del modulo' . "\n");
								# Definiendo constantes de cada una de las tablas
								fwrite($var_file, "	define('url_delete_" . $tables[0] . "',  str_replace('_', '', '" . $tables[0] . "/eliminate/delete/'));" . "\n");
								fwrite($var_file, "	define('url_remove_" . $tables[0] . "',  str_replace('_', '', '" . $tables[0] . "/info/remove/'));" . "\n");
								fwrite($var_file, "	define('url_restore_" . $tables[0] . "',  str_replace('_', '', '" . $tables[0] . "/info/restore/'));" . "\n");
								fwrite($var_file, "	define('url_query_" . $tables[0] . "',  str_replace('_', '', '" . $tables[0] . "/info/query/'));" . "\n");
							endforeach;
							# Escritura de un archivo en modo binario seguro
							fwrite($var_file, '?>');
							fclose($var_file);
							# Redireccionar a una pagina segun el metodo cargado
							Func::redir(URL . $var_controller . 'info/update/');
						} else {
							# Redireccionar a una pagina segun el metodo cargado
					...
					case 'delete':
						...
						if (false != $var_m_zz_design->sql_permits_value_delete($var_controller)) {
							...
							if ($this->isset_id) {
								...
								$var_m_azp_menu_crud->sql_controller_delete();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_menu_crud->sql_azp_menu_crud_delete();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
					...
					case 'info':
						...
						if (false != $var_m_zz_design->sql_permits_menu_value($var_controller)) {
							# Cargar en el metodo por url la plantilla con su contenido a visualizar
							echo $this->template->render('backend/inward/azp_menu_crud', array(
								# Consultar registros de la tabla por medio del modelo instanciado
								...
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_application' => $var_m_azp_menu_crud->sql_select_tc_azp_application(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_icon_code' => $var_m_azp_menu_crud->sql_select_tc_azp_icon_code(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_table_prefix' => $var_m_azp_menu_crud->sql_select_tc_azp_table_prefix(),
					...
					case 'remove':
						...
						if (false != $var_m_zz_design->sql_permits_value_remove($var_controller)) {
							...
							if ($this->isset_id) {
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_menu_crud->sql_controller_remove();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_menu_crud->sql_azp_menu_crud_remove();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'info/query/');
								# Detener la ejecucion
					...
					case 'restore':
						...
						if (false != $var_m_zz_design->sql_permits_value_restore($var_controller)) {
							...
							if ($this->isset_id) {
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_menu_crud->sql_controller_restore();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_menu_crud->sql_azp_menu_crud_restore();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'info/query/');
								# Detener la ejecucion
