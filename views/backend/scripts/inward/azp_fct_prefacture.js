...
if (document.getElementById('form_btn_invoice')) {
	document.getElementById('form_btn_invoice').onclick = function(e) {
		e.preventDefault();
		event_invoice();
	};
}
...
if (document.getElementById('form_invoice')) {
	document.getElementById('form_invoice').onkeypress = function(e) {
		if (!e) e = window.event;
		var keyCode = e.keyCode || e.which;
		if (keyCode == '13') {
			event_invoice();
...
/*function modal_calculate() {
	$('#modal-id-calculate').modal('show');

	var vcrt = document.getElementById('field_ct_certificate').value;
	var vdfc = document.getElementById('field_ct_defunction').value;
	var vibn = document.getElementById('field_ct_investigation').value;

	console.log(vcrt);
	console.log(vdfc);
	console.log(vibn);
	
	$('#check_ct_certificate').prop('checked', true);
	$('#check_ct_defunction').prop('checked', true);
	$('#check_ct_investigation').prop('checked', true);
...
function event_change_data() {
	...
	if (idtbl > 0) {
		...
		var columnSet = [
			{
				title: "Id",
				id: "lbl_azs_fct_prefacture_id_azs_fct_prefacture",
				data: "lbl_azs_fct_prefacture_id_azs_fct_prefacture",
				type: "readonly",
				className: "text-dark",
				visible: true,
			},
			...
			},
			{
				...
				id: "lbl_azs_fct_prefacture_m_date_created",
				data: "lbl_azs_fct_prefacture_m_date_created",
				type: "readonly",
				className: "text-dark",
				visible: false,
			},
			{
				...
				id: "lbl_azs_fct_prefacture_m_date_modified",
				data: "lbl_azs_fct_prefacture_m_date_modified",
				type: "readonly",
				className: "text-dark",
				visible: false,
			},
			{
				title: "Descripcion",
				id: "lbl_azs_fct_prefacture_m_description",
				data: "lbl_azs_fct_prefacture_m_description",
				type: "readonly",
				className: "text-dark",
				visible: false,
			},
			...
			{
				title: "Nombre",
				id: "lbl_azs_fct_prefacture_t_name",
				data: "lbl_azs_fct_prefacture_t_name",
				type: "readonly",
				className: "text-dark",
				visible: true,
			},
		...
		buttons: {
			...
			ok: {
				...
				callback: function() {
					$.ajax({
						type : "POST",
						url : "api/azp_fct_prefacture/create",
		...
		buttons: {
			...
			ok: {
				...
				callback: function() {
					$.ajax({
						type : "POST",
						url : "api/azp_fct_prefacture/delete",
	...
	if (idtbl > 0) {
		...
		request.onreadystatechange = function(){
			if (request.readyState == 4 && request.status == 200) {
				...
				if (objData.status) {
					$('#modal-id-detail').modal('show');
					document.querySelector("#lbl_azp_fct_prefacture_id_azp_fct_prefacture").innerHTML = objData.data.lbl_azp_fct_prefacture_id_azp_fct_prefacture;
					...
					document.querySelector("#lbl_azp_fct_prefacture_m_date_created").innerHTML = objData.data.lbl_azp_fct_prefacture_m_date_created;
					document.querySelector("#lbl_azp_fct_prefacture_m_date_modified").innerHTML = objData.data.lbl_azp_fct_prefacture_m_date_modified;
					document.querySelector("#lbl_azp_fct_prefacture_m_description").innerHTML = objData.data.lbl_azp_fct_prefacture_m_description;
					document.querySelector("#lbl_azp_sys_state_t_name").innerHTML = objData.data.lbl_azp_sys_state_t_name;
					document.querySelector("#lbl_azp_fct_prefacture_t_name").innerHTML = objData.data.lbl_azp_fct_prefacture_t_name;
					document.querySelector("#lbl_azp_fct_service_t_name").innerHTML = objData.data.lbl_azp_fct_service_t_name;
				} else {
					// Mensaje de audio para mostrar en la pagina web
					initApp.playSound('views/templates/smart/dist/media/sound', 'voice_on');
					bootbox.alert({
		...
		buttons: {
			...
			ok: {
				...
				callback: function() {
					$.ajax({
						type : "POST",
						url : "api/azp_fct_prefacture/remove",
		...
		buttons: {
			...
			ok: {
				...
				callback: function() {
					$.ajax({
						type : "POST",
						url : "api/azp_fct_prefacture/restore",
		...
		buttons: {
			...
			ok: {
				...
				callback: function() {
					$.ajax({
						type : "POST",
						url : "api/azp_fct_prefacture/update",
	...
	if (idtbl > 0) {
		...
		request.onreadystatechange = function(){
			if (request.readyState == 4 && request.status == 200) {
				...
				if (objData.status) {
					$('#modal-id-update').modal('show');
					document.querySelector("#field_up_id").value = objData.data.lbl_azp_fct_prefacture_id_azp_fct_prefacture;
					document.querySelector("#field_up_description").value = objData.data.lbl_azp_fct_prefacture_m_description;
					document.querySelector("#field_up_state").value = objData.data.lbl_azp_fct_prefacture_m_status;
					document.querySelector("#field_up_identification").value = objData.data.lbl_azp_fct_prefacture_t_identification;
					document.querySelector("#field_up_tariff").value = objData.data.lbl_azp_fct_prefacture_v_id_azp_fct_tariff;
				} else {
					// Mensaje de audio para mostrar en la pagina web
					initApp.playSound('views/templates/smart/dist/media/sound', 'voice_on');
					bootbox.alert({
...
/*function modal_calculate() {
	...
	var ajaxurl = var_url + 'api' + var_controller + "calculate/";
	$.ajax({ type: 'POST', url: ajaxurl, }).done(function(argument) {
		...
		// Mensaje a mostrar una vez terminada la accion
		$('#form_ajax_calculate').html(success_icon + 'Exito: Datos procesados correctamente.');
		$("#form_ajax_calculate").removeClass('alert-warning');
		...
		bootbox.alert({
			title: "<i class='fal fa-check-circle text-success mr-2'></i> <span class='text-success fw-500'>Completado!</span>",
			message: "<span><strong>Exito:</strong> Hemos actualizado exitosamente los datos.</span>",
			centerVertical: true,
			className: "modal-alert",
			closeButton: false,
		});
		...
		request.onreadystatechange = function(){
			if (request.readyState == 4 && request.status == 200) {
				...
				if (objData.status) {
					...
					var idfct = objData.data.lbl_azp_fct_prefacture_id_azp_fct_prefacture;
					var service = objData.data.lbl_azp_fct_service_t_name;
					var investigation = objData.data.lbl_azp_fct_investigation_t_name;
					...
					if (service == 'CERTIFICADOS') {
						...
						var myTable = $('#dt_certificate').dataTable({
							...
							dom: "<'row mb-3'<'col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 d-flex align-items-center'B>>"
								...
								+ "<'row mb-3'<'col-xs-12 col-sm-12 col-md-5 col-lg-4 col-xl-4'i><'col-xs-12 col-sm-12 col-md-7 col-lg-8 col-xl-8'p>>",
							// Leer datos de prácticamente cualquier fuente de datos JSON
							ajax: var_url + 'api' + var_controller + "certificate/" + idfct + "/",
							// Cargar columnas del encabezado
							columns: columnSet,
							// Opciones para crear, editar, borrar y sincronizar
							altEditor: true,
					...
					if (service == 'DEFUNCION') {
						...
						var columnSet = [
							...
							{
								title: "Fecha Solicitud",
								id: "lbl_azp_gsc_request_t_request_date",
								data: "lbl_azp_gsc_request_t_request_date",
								type: "readonly",
								className: "text-dark",
								visible: true,
							},
							{
								title: "Fecha Entrega",
								id: "lbl_azp_gsc_request_t_shipping_date",
								data: "lbl_azp_gsc_request_t_shipping_date",
								type: "readonly",
								className: "text-dark",
								visible: true,
							},
							...
							{
								title: "Nombre",
								id: "lbl_azp_gsc_request_t_full_name",
								data: "lbl_azp_gsc_request_t_full_name",
								type: "readonly",
								className: "text-dark",
								visible: true,
							},
							{
								title: "Obligacion",
								id: "lbl_azp_gsc_request_t_number_obligation",
								data: "lbl_azp_gsc_request_t_number_obligation",
								type: "readonly",
								className: "text-dark",
								visible: true,
							},
							...
							{
								title: "Serial",
								id: "lbl_azp_gsc_request_t_serial",
								data: "lbl_azp_gsc_request_t_serial",
								type: "readonly",
								className: "text-dark",
								visible: true,
							},
							...
							{
								title: "Total",
								id: "lbl_azp_fct_tariff_t_total",
								data: "lbl_azp_fct_tariff_t_total",
								type: "text",
								className: "text-dark",
								visible: true,
							},
					...
					if (service == 'INVESTIGACION BIENES') {
						...
						if (investigation == 'EFECTIVIDAD') {
							...
							var columnSet = [
								...
								{
									title: "Ciudad",
									id: "lbl_azp_ibn_evaluation_t_municipality",
									data: "lbl_azp_ibn_evaluation_t_municipality",
									type: "readonly",
									className: "text-dark",
									visible: true,
								},
								/*{
									...
									id: "lbl_azp_ibn_data_main_plaque",
									data: "lbl_azp_ibn_data_main_plaque",
									type: "readonly",
									className: "text-dark",
									visible: true,
								},*/
								{
									title: "Consecutivo",
									id: "lbl_azp_ibn_data_main_t_consecutive",
									data: "lbl_azp_ibn_data_main_t_consecutive",
									type: "readonly",
									className: "text-dark",
									visible: true,
								},
								{
									title: "Resultado",
									id: "lbl_azp_ibn_evaluation_t_policy_effective",
									data: "lbl_azp_ibn_evaluation_t_policy_effective",
									type: "readonly",
									className: "text-dark",
									visible: true,
								},
							...
							var myTable = $('#dt_investigation').dataTable({
								...
								dom: "<'row mb-3'<'col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 d-flex align-items-center'B>>"
									...
									+ "<'row mb-3'<'col-xs-12 col-sm-12 col-md-5 col-lg-4 col-xl-4'i><'col-xs-12 col-sm-12 col-md-7 col-lg-8 col-xl-8'p>>",
								// Leer datos de prácticamente cualquier fuente de datos JSON
								ajax: var_url + 'api' + var_controller + "investigation/" + idfct + "/",
								// Cargar columnas del encabezado
								columns: columnSet,
								// Opciones para crear, editar, borrar y sincronizar
								altEditor: true,
						...
						if (investigation == 'VIABILIDAD') {
							...
							var columnSet = [
								...
								{
									title: "Resultado",
									id: "lbl_azp_ibn_evaluation_t_policy_viability",
									data: "lbl_azp_ibn_evaluation_t_policy_viability",
									type: "readonly",
									className: "text-dark",
									visible: true,
								},
...
/*function updateTimer() {
	...
	// Formatea los valores de tiempo en dos dígitos (por ejemplo, 01:02:03)
	var timeString = padNumber(hours, 2) + ':' + padNumber(minutes, 2) + ':' + padNumber(seconds, 2);

	var message = $('#form_ajax_calculate').html(process_icon + 'Advertencia: Procesando información, por favor espere... ' + timeString);
...
function event_calculate() {
	...
	var value_table = document.querySelectorAll('.dt_tb_client');
	...
	if (value_checked == '') {
		// Mensaje a mostrar una vez terminada la accion
		$('#form_ajax_calculate').html(error_icon + 'Error: Debes seleccionar minimo un cliente');
		$("#form_ajax_calculate").removeClass('alert-warning');
		...
		function updateTimer() {
			...
			// Actualiza el contenido del elemento HTML
			$('#form_ajax_calculate').html(process_icon + 'Advertencia: Procesando información, por favor espere... ' + timeString);
		}

		// Función para agregar un cero delante de los números de un solo dígito
		function padNumber(number, length) {
			var str = '' + number;
			while (str.length < length) {
				str = '0' + str;
		...
		// Función para convertir el tiempo en segundos a formato de horas, minutos y segundos
		...
		$.ajax({
			type : "POST",
			url : "api/azp_fct_prefacture/calculate",
			data : $('#form_calculate').serialize() + '&field_ct_client=' + value_checked,
			success : function(json) {
				var obj = jQuery.parseJSON(json);
				if (obj.success == 1) {
					// Recargar la tabla con nueva informacion
					...
					var duration = convertTime(elapsedTime);
					$('#form_ajax_info').html('');
					$('#form_ajax_info').removeClass('hide');
					$("#form_ajax_info").removeClass('alert-danger');
					$("#form_ajax_info").removeClass('alert-warning');
					$('#form_ajax_info').addClass('alert');
					$("#form_ajax_info").addClass('alert-success');
					$('#form_ajax_info').html(success_icon + obj.message + ' <b>Duración de Ejecucion:</b>' + ' ' + duration);
			...
			if (requestacl.readyState == 4 && requestacl.status == 200) {
				...
				if (objDataacl.status) {
					...
					if (cmax > 1) {
						...
						$('#modal-id-invoice').modal('show');
						// Dejar campos vacios sobre los elementos id del formulario
						document.querySelector("#form_invoice").reset();
					}
				} else {
					initApp.playSound('views/templates/smart/dist/media/sound', 'voice_on');
					bootbox.alert({
...
function event_invoice() {
	...
	var process_icon = '<span class="far fa-spinner fa-spin" aria-hidden="true"></span>' + ' ';
	// Añadir y remover las clases incorporadas de las hojas de estilo
	$('#form_ajax_invoice').removeClass('alert-danger');
	$('#form_ajax_invoice').removeClass('alert-success');
	$('#form_ajax_invoice').removeClass('alert-warning');
	// Añadir y remover las clases incorporadas de las hojas de estilo
	$('#form_ajax_invoice').addClass('alert-warning');
	$("#form_ajax_invoice").html(process_icon  + 'Advertencia: Procesando información, por favor espere...');
	$('#form_ajax_invoice').removeClass('hide');
	$('#form_ajax_invoice').addClass('alert');
	// Configuracion de la ventana modal
	//$('#modal-id-invoice').modal('hide');
	// Obtener inputs seleccionados de la tabla
	var value_table = document.querySelectorAll('.dt_tb_info');
	// Array para cargar los valores obtenidos
	var value_checked = new Array();
			...
			buttons: {
				cancel: {
					...
					callback: function() {
						// Añadir y remover las clases incorporadas de las hojas de estilo
						setTimeout(function() {
							$('#form_ajax_invoice').removeClass('alert');
							$('#form_ajax_invoice').addClass('hide');
							$('#form_ajax_invoice').html('');
						}, 0);
						// Mensaje de audio para mostrar en la pagina web
						initApp.playSound('views/templates/smart/dist/media/sound', 'voice_on');
						bootbox.alert({
				...
				ok: {
					...
					callback: function() {
						$.ajax({
							type : "POST",
							url : "api/azp_fct_prefacture/invoice",
							data : $('#form_invoice').serialize() + '&field_iv_check=' + value_checked,
							success : function(json) {
								var obj = jQuery.parseJSON(json);
								if (obj.success == 1) {
									// Recargar la tabla con nueva informacion
									...
									if ($("#dt_group").length) { $('#dt_group').DataTable().ajax.reload(); }
									// Mensaje a mostrar una vez terminada la accion
									$('#form_ajax_invoice').html(success_icon + obj.message);
									$("#form_ajax_invoice").removeClass('alert-warning');
									$("#form_ajax_invoice").addClass('alert-success');
									// Dejar campos vacios sobre los elementos id del formulario
									document.querySelector("#form_invoice").reset();
									// Añadir y remover las clases incorporadas de las hojas de estilo
									setTimeout(function() {
									...
									// Mensaje a mostrar una vez terminada la accion
									$('#form_ajax_invoice').html(error_icon + obj.message);
									$("#form_ajax_invoice").removeClass('alert-warning');
									$("#form_ajax_invoice").addClass('alert-danger');
								}
							},
							error : function() {
								$('#form_ajax_invoice').addClass('alert-danger');
								$("#form_ajax_invoice").html(process_icon  + 'Error: Problema detectado en el sistema, por favor comuniquese con el administrador');
								$('#form_ajax_invoice').removeClass('hide');
							},
						});
					},
...
function event_checkin() {
	// Visualizar ventana modal o interfaz
	$('#modal-id-checkin').modal('show');
	// Column Definitions
	var columnSet = [
		{
			title: "Id",
			id: "lbl_azp_fct_invoice_id_azp_fct_invoice",
			data: "lbl_azp_fct_invoice_id_azp_fct_invoice",
			type: "readonly",
			className: "text-dark",
			visible: false,
		},
		{
			...
			id: "lbl_azp_fct_invoice_t_number",
			data: "lbl_azp_fct_invoice_t_number",
			type: "readonly",
			className: "text-dark lbl_azp_fct_invoice_t_number",
			visible: true,
		},
		{
			title: "Bases Facturadas",
			id: "lbl_azp_fct_invoice_t_count",
			data: "lbl_azp_fct_invoice_t_count",
			type: "readonly",
			className: "text-dark",
			visible: true,
		},
	...
	// Inicializar tabla de datos
	var myTable = $('#dt_checkin').dataTable({
		// Reinicialiar los datos de la tabla
		destroy: true,
		// Estructura de las columnas para el DOOM
		dom: "<'row mb-3'<'col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 d-flex align-items-center'B>>"
			...
			+ "<'row mb-3'<'col-xs-12 col-sm-12 col-md-5 col-lg-4 col-xl-4'i><'col-xs-12 col-sm-12 col-md-7 col-lg-8 col-xl-8'p>>",
		// Leer datos de prácticamente cualquier fuente de datos JSON
		ajax: var_url + 'api' + var_controller + "checkin/",
		// Cargar columnas del encabezado
		columns: columnSet,
		// Opciones para crear, editar, borrar y sincronizar
		altEditor: true,
		...
		buttons: [
			...
			{
				text: '<i class="far fa-barcode-read"></i>',
				titleAttr: 'Consultar',
				className: 'btn-sm btn-outline-dark',
				attr: {
					'id': 'table_btn_turnover',
					'name': 'table_btn_turnover',
					'onclick': 'event_turnover();',
				},
			},
		],
		columnDefs: [],
...
function event_turnover() {
	// Mostrar la columna especificada de la tabla
	//var vtable = $('#dt_invoice').DataTable();
	//vtable.column(0).visible(true);
	// Visualizar ventana modal o interfaz
	$('#modal-id-turnover').modal('show');
	// Obtener el primer valor seleccionado de la tabla
	//var idtbl = $("#dt_invoice tr.selected td:first").html();
	var idtbl = $("#dt_checkin tr.selected td.lbl_azp_fct_invoice_t_number").html();
	console.log(idtbl);
	// Validar si el id es mayor a cero
	if (idtbl == 'undefined' || idtbl == '') {
		...
		var columnSet = [
			...
			{
				title: "Fecha Creacion",
				id: "lbl_azp_fct_invoice_m_date_created",
				data: "lbl_azp_fct_invoice_m_date_created",
				type: "text",
				className: "text-dark",
				visible: false,
			},
			{
				title: "Fecha Modificacion",
				id: "lbl_azp_fct_invoice_m_date_modified",
				data: "lbl_azp_fct_invoice_m_date_modified",
				type: "text",
				className: "text-dark",
				visible: false,
			},
			{
				title: "Descripcion",
				id: "lbl_azp_fct_invoice_m_description",
				data: "lbl_azp_fct_invoice_m_description",
				type: "text",
				className: "text-dark",
				visible: false,
			},
			...
			{
				title: "Solicitud",
				id: "lbl_azp_fct_invoice_t_request",
				data: "lbl_azp_fct_invoice_t_request",
				type: "text",
				className: "text-dark",
				visible: true,
			},
			{
				title: "Fecha",
				id: "lbl_azp_fct_invoice_t_date_request",
				data: "lbl_azp_fct_invoice_t_date_request",
				type: "text",
				className: "text-dark",
				visible: true,
			},
			...
			{
				title: "Emitidos",
				id: "lbl_azp_fct_invoice_t_purchase",
				data: "lbl_azp_fct_invoice_t_purchase",
				type: "text",
				className: "text-dark",
				visible: true,
			},
			{
				title: "Tarifas",
				id: "lbl_azp_fct_invoice_s_value",
				data: "lbl_azp_fct_invoice_s_value",
				type: "text",
				className: "text-dark",
				visible: true,
			},
			{
				title: "Servicios",
				id: "lbl_azp_fct_invoice_t_honorary",
				data: "lbl_azp_fct_invoice_t_honorary",
				type: "text",
				className: "text-dark",
				visible: true,
			},
		...
		// Inicializar tabla de datos
		var myTable = $('#dt_turnover').dataTable({
			// Reinicialiar los datos de la tabla
			destroy: true,
			// Estructura de las columnas para el DOOM
			dom: "<'row mb-3'<'col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 d-flex align-items-center'B>>"
				...
				+ "<'row mb-3'<'col-xs-12 col-sm-12 col-md-5 col-lg-4 col-xl-4'i><'col-xs-12 col-sm-12 col-md-7 col-lg-8 col-xl-8'p>>",
			// Leer datos de prácticamente cualquier fuente de datos JSON
			ajax: var_url + 'api' + var_controller + "turnover/?field_number=" + idtbl + "",
			// Cargar columnas del encabezado
			columns: columnSet,
			// Opciones para crear, editar, borrar y sincronizar
			altEditor: true,
	...
	if (value_checked == '') {
		...
		//$.ajax({ type: 'POST', url: ajaxclient, }).done(function(data) {
			...
			$.ajax({ type: 'POST', url: ajaxurl, }).done(function(argument) {
				...
				var a = $("<a>").attr("href", argument).attr("download", 'Reporte Prefactura').appendTo("body");
				a[0].click(); a.remove();
				// Recargar la tabla con nueva informacion
				if ($("#dt_info").length) { $('#dt_info').DataTable().ajax.reload(); }
				if ($("#dt_group").length) { $('#dt_group').DataTable().ajax.reload(); }
...
function event_table_client() {
	// Control de validacion del checkbox seleccionado
	if ($('#field_ct_table_client').is(':checked')) {
		// Marcar todos los checkbox
		$('.dt_tb_client').prop('checked', true);
	} else {
		// Marcar todos los checkbox
		$('.dt_tb_client').prop('checked', false);
	}
};