<?php
	...
	//------------------------------------------------
	class azpcrtcaptureController extends Controllers {
		public function __construct() {
			...
			$var_m_azp_crt_capture = new m_azp_crt_captureModel;
			# Instanciando clases del modelo
			...
			if (NULL != $var_get_method && Strings::only_letters($var_get_method)) {
				...
				switch ($var_get_method) {
					case 'certificate':
						...
						$var_id = $_POST['field_ct_id'];
						$var_name = $_FILES['field_ct_file']['name'];
						$var_type = $_FILES['field_ct_file']['type'];
						$var_tmp_name = $_FILES['field_ct_file']['tmp_name'];
						$var_error = $_FILES['field_ct_file']['error'];
						$var_size = $_FILES['field_ct_file']['size'];
						# Enviar datos a traves del modelo
						$var_m_azp_crt_capture->sql_azp_crt_capture_fcertificate($var_id, $var_name, $var_type, $var_tmp_name, $var_error, $var_size);
						break;
					case 'receipt':
						# Capturar datos por el metodo post
						$var_id = $_POST['field_rb_id'];
						$var_nro = $_POST['field_rb_nro_receipt'];
						$var_name = $_FILES['field_rb_file']['name'];
						$var_type = $_FILES['field_rb_file']['type'];
						$var_tmp_name = $_FILES['field_rb_file']['tmp_name'];
						$var_error = $_FILES['field_rb_file']['error'];
						$var_size = $_FILES['field_rb_file']['size'];
						# Enviar datos a traves del modelo
						$var_m_azp_crt_capture->sql_azp_crt_capture_freceipt($var_id, $var_nro, $var_name, $var_type, $var_tmp_name, $var_error, $var_size);
						break;
					case 'access':
					...
					case 'delete':
						...
						if (false != $var_m_zz_design->sql_permits_value_delete($var_controller)) {
							...
							if ($this->isset_id) {
								...
								$var_m_azp_crt_capture->sql_controller_delete();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_crt_capture->sql_azp_crt_capture_delete();
							} else {
					...
					case 'info':
						...
						if (false != $var_m_zz_design->sql_permits_menu_value($var_controller)) {
							# Cargar en el metodo por url la plantilla con su contenido a visualizar
							echo $this->template->render('backend/inward/azp_crt_capture', array(
								# Consultar registros de la tabla por medio del modelo instanciado
					...
					case 'remove':
						...
						if (false != $var_m_zz_design->sql_permits_value_remove($var_controller)) {
							...
							if ($this->isset_id) {
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_crt_capture->sql_controller_remove();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_crt_capture->sql_azp_crt_capture_remove();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'info/query/');
								# Detener la ejecucion
					...
					case 'restore':
						...
						if (false != $var_m_zz_design->sql_permits_value_restore($var_controller)) {
							...
							if ($this->isset_id) {
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_crt_capture->sql_controller_restore();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_crt_capture->sql_azp_crt_capture_restore();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'info/query/');
								# Detener la ejecucion
