<?php
	...
	//------------------------------------------------
	class azpfdpbenefitsController extends Controllers {
			...
			if (NULL != $var_get_method && Strings::only_letters($var_get_method)) {
				...
				switch ($var_get_method) {
					...
					case 'delete':
						...
						if (false != $var_m_zz_design->sql_permits_value_delete($var_controller)) {
							...
							if ($this->isset_id) {
								...
								$var_m_azp_fdp_benefits->sql_controller_delete();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_fdp_benefits->sql_azp_fdp_benefits_delete();
							} else {
					...
					case 'import':
						...
						$var_name = $_FILES['field_file_excel']['name'];
						$var_type = $_FILES['field_file_excel']['type'];
						$var_tmp_name = $_FILES['field_file_excel']['tmp_name'];
						$var_error = $_FILES['field_file_excel']['error'];
						$var_size = $_FILES['field_file_excel']['size'];

						# Enviar datos a traves del modelo
						$var_m_azp_fdp_benefits->sql_azp_fdp_benefits_extra_file($var_name, $var_type, $var_tmp_name, $var_error, $var_size);
						break;
					case 'info':
						# Control para validar el acceso a los resultados de la base de datos
						if (false != $var_m_zz_design->sql_permits_menu_value($var_controller)) {
							# Cargar en el metodo por url la plantilla con su contenido a visualizar
							echo $this->template->render('backend/inward/azp_fdp_benefits', array(
								# Consultar registros de la tabla por medio del modelo instanciado
					...
					case 'remove':
						...
						if (false != $var_m_zz_design->sql_permits_value_remove($var_controller)) {
							...
							if ($this->isset_id) {
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_fdp_benefits->sql_controller_remove();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_fdp_benefits->sql_azp_fdp_benefits_remove();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'info/query/');
								# Detener la ejecucion
					...
					case 'restore':
						...
						if (false != $var_m_zz_design->sql_permits_value_restore($var_controller)) {
							...
							if ($this->isset_id) {
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_fdp_benefits->sql_controller_restore();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_fdp_benefits->sql_azp_fdp_benefits_restore();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'info/query/');
								# Detener la ejecucion
