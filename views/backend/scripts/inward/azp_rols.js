...
function event_change_data() {
	...
	if (idtbl > 0) {
		...
		var columnSet = [
			{
				title: "Id",
				id: "lbl_azs_rols_id_azs_rols",
				data: "lbl_azs_rols_id_azs_rols",
				type: "readonly",
				className: "text-dark",
				visible: true,
			},
			...
			},
			{
				...
				id: "lbl_azs_rols_m_date_created",
				data: "lbl_azs_rols_m_date_created",
				type: "readonly",
				className: "text-dark",
				visible: false,
			},
			{
				...
				id: "lbl_azs_rols_m_date_modified",
				data: "lbl_azs_rols_m_date_modified",
				type: "readonly",
				className: "text-dark",
				visible: true,
			},
			{
				title: "Descripcion",
				id: "lbl_azs_rols_m_description",
				data: "lbl_azs_rols_m_description",
				type: "readonly",
				className: "text-dark",
				visible: false,
			},
			...
			{
				title: "Nombre",
				id: "lbl_azs_rols_t_name",
				data: "lbl_azs_rols_t_name",
				type: "readonly",
				className: "text-dark",
				visible: true,
			},
	...
	if (idtbl > 0) {
		...
		request.onreadystatechange = function(){
			if (request.readyState == 4 && request.status == 200) {
				...
				if (objData.status) {
					$('#modal-id-detail').modal('show');
					document.querySelector("#lbl_azp_rols_id_azp_rols").innerHTML = objData.data.lbl_azp_rols_id_azp_rols;
					...
					document.querySelector("#lbl_azp_rols_m_date_created").innerHTML = objData.data.lbl_azp_rols_m_date_created;
					document.querySelector("#lbl_azp_rols_m_date_modified").innerHTML = objData.data.lbl_azp_rols_m_date_modified;
					document.querySelector("#lbl_azp_rols_m_description").innerHTML = objData.data.lbl_azp_rols_m_description;
	...
	if (idtbl > 0) {
		...
		request.onreadystatechange = function(){
			if (request.readyState == 4 && request.status == 200) {
				...
				if (objData.status) {
					$('#modal-id-update').modal('show');
					document.querySelector("#field_up_id").value = objData.data.lbl_azp_rols_id_azp_rols;
					document.querySelector("#field_up_description").value = objData.data.lbl_azp_rols_m_description;
					document.querySelector("#field_up_state").value = objData.data.lbl_azp_rols_m_status;
					document.querySelector("#field_up_name").value = objData.data.lbl_azp_rols_t_name;
				} else {
					// Mensaje de audio para mostrar en la pagina web
					initApp.playSound('views/templates/smart/dist/media/sound', 'voice_on');
					bootbox.alert({
