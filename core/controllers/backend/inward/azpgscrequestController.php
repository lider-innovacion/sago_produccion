<?php
	...
	//------------------------------------------------
	class azpgscrequestController extends Controllers {
				...
				switch ($var_get_method) {
					...
					case 'delete':
						...
						if (false != $var_m_zz_design->sql_permits_value_delete($var_controller)) {
							...
							if ($this->isset_id) {
								...
								$var_m_azp_gsc_request->sql_controller_delete();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_gsc_request->sql_azp_gsc_request_delete();
							} else {
					...
					case 'document':
						...
						if (false != $var_m_zz_design->sql_permits_value_import($var_controller)) {
							...
							$var_m_azp_gsc_request->sql_azp_gsc_request_document($var_template, $var_causal, $var_name, $var_type, $var_tmp_name, $var_error, $var_size);
						} else {
							# Redireccionar a una pagina segun el metodo cargado
							Func::redir(URL . $var_controller . 'info/access/');
							# Detener la ejecucion
					...
					case 'zipimages':
						# Enviar datos a traves del modelo
						$var_function = $var_m_azp_gsc_request->sql_azp_gsc_compressed_images();
						# Recorriendo filas de los resultados de la base de datos
						foreach ($var_function as $key => $value): echo $value; endforeach;
						break;
					case 'zipimagescl':
						# Enviar datos a traves del modelo
						$var_function = $var_m_azp_gsc_request->sql_azp_gsc_compressed_images_client();
						# Recorriendo filas de los resultados de la base de datos
						foreach ($var_function as $key => $value): echo $value; endforeach;
						break;
					case 'info':
						...
						if (false != $var_m_zz_design->sql_permits_menu_value($var_controller)) {
							...
							# Consultar registros de la tabla por medio del modelo instanciado
							$var_m_azp_gsc_request->sql_azp_gsc_request_data();
							# Cargar en el metodo por url la plantilla con su contenido a visualizar
							echo $this->template->render('backend/inward/azp_gsc_request', array(
								# Consultar registros de la tabla por medio del modelo instanciado
								...
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_gsc_phase_percentage' => $var_m_azp_gsc_phase->sql_controller_read(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_causal' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_causal(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_causales' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_causales(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_city' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_city(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_client' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_client(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_department' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_department(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_phase' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_phase(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_third_parties' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_third_parties(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_third_parties_group_by' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_third_parties_group_by(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_gsc_request_client' => $var_m_azp_gsc_request->sql_select_tc_azp_gsc_request_client(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_controller_ultime' => $var_m_azp_gsc_request->sql_controller_ultime(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'df_sql_select_tc_azp_fct_client' => $var_m_azp_gsc_request->sql_select_tc_azp_fct_client(),
								# Consultar registros de la tabla por medio del modelo instanciado
								'sql_select_tc_azp_fct_client_bs' => $var_m_azp_gsc_request->sql_select_tc_azp_fct_client(),
								# Consultar registros de la tabla por medio del modelo instanciado
					...
					case 'remove':
						...
						if (false != $var_m_zz_design->sql_permits_value_remove($var_controller)) {
							...
							if ($this->isset_id) {
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_gsc_request->sql_controller_remove();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_gsc_request->sql_azp_gsc_request_remove();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'info/query/');
								# Detener la ejecucion
					...
					case 'restore':
						...
						if (false != $var_m_zz_design->sql_permits_value_restore($var_controller)) {
							...
							if ($this->isset_id) {
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_gsc_request->sql_controller_restore();
								# Consultar registros de la tabla por medio del modelo instanciado
								$var_m_azp_gsc_request->sql_azp_gsc_request_restore();
							} else {
								# Redireccionar a una pagina segun el metodo cargado
								Func::redir(URL . $var_controller . 'info/query/');
								# Detener la ejecucion
					...
					case 'xlsxemailone':
						...
						#-------------------------------------------------------------------------#
						# Instanciado funciones de la clase y asignando a variable
						//$var_m_azp_gsc_request->sql_select_tc_azp_gsc_third_type_up($var_em_id, $var_em_nro);
						# Instanciando clases del modelo
						$var_spreadsheet = new Spreadsheet();
						# Instanciando clases del modelo
						$var_sheet = $var_spreadsheet->getActiveSheet();
						# Instanciado funciones de la clase y asignando a variable
						$var_sql_table = $var_m_azp_gsc_request->sql_controller_table();
						# Instanciado funciones de la clase y asignando a variable
						$var_sql_password = $var_m_azp_gsc_request->sql_controller_third_password($var_em_id);
						#-------------------------------------------------------------------------#
						# Ruta principal de la carpeta
						$var_fl_route = 'systemfiles/filesproject/';
						# Ruta de la carpeta principal
						$var_fl_folder = 'azp_gsc_request/supplier/';
						# Ruta de la carpeta principal
						...
						$var_sheet->setCellValue('A1', html_entity_decode('Nro Consecutivo'));
						$var_sheet->setCellValue('B1', html_entity_decode('Id Proveedor'));
						$var_sheet->setCellValue('C1', html_entity_decode('Cedula'));
						$var_sheet->setCellValue('D1', html_entity_decode('Nombre'));
						$var_sheet->setCellValue('E1', html_entity_decode('Serial'));
						$var_sheet->setCellValue('F1', html_entity_decode('Notaria'));
						$var_sheet->setCellValue('G1', html_entity_decode('Ciudad'));
						$var_sheet->setCellValue('H1', html_entity_decode('Causal'));
						$var_sheet->setCellValue('I1', html_entity_decode('Fecha Defuncion'));
						$var_sheet->setCellValue('J1', html_entity_decode('Observacion Externa'));
						#-------------------------------------------------------------------------#
						...
						if (false != $var_sql_table) {
							...
							foreach ($var_sql_table as $key => $value) { $var_item = $var_acount += 1;
								# Asignar valores a las celdas de excel
								$var_sheet->setCellValue('A' . $i, html_entity_decode($var_item));
								$var_sheet->setCellValue('B' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_id_supplier']));
								$var_sheet->setCellValue('C' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_identification']));
								$var_sheet->setCellValue('D' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_full_name']));
								#$var_sheet->setCellValue('E' . $i, preg_replace('/^0+/', '', str_replace('_', '', html_entity_decode($value['lbl_azp_gsc_update_t_serial']))));
								#$var_sheet->setCellValue('F' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_notary']));
								#$var_sheet->setCellValue('G' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_city']));
								#$var_sheet->setCellValue('H' . $i, '');
								#$var_sheet->setCellValue('I' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_death_date']));
								#$var_sheet->setCellValue('J' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_observation_outer']));
								$var_sheet->setCellValue('E' . $i, '');
								$var_sheet->setCellValue('F' . $i, '');
								$var_sheet->setCellValue('G' . $i, '');
								$var_sheet->setCellValue('H' . $i, '');
								$var_sheet->setCellValue('I' . $i, '');
								$var_sheet->setCellValue('J' . $i, '');
								# Incrementar valores
								$i++;
							}
						} else {
						...
						if ($var_password != '') {
							...
							exec($encryptCommand);
							# Enviar datos a traves del modelo
							$var_m_azp_gsc_request->sql_azp_gsc_request_generated_files(
								$var_expiration,
								$var_extension,
								$var_module,
								$var_encry_name,
					...
					case 'xlsxemailfour':
						...
						#-------------------------------------------------------------------------#
						# Instanciado funciones de la clase y asignando a variable
						$var_sql_client = $var_m_azp_gsc_request->sql_controller_client($var_em_id);
						# Instanciado funciones de la clase y asignando a variable
						$var_sql_table = $var_m_azp_gsc_request->sql_controller_table();
						# Instanciado funciones de la clase y asignando a variable
						//$var_sql_table_up = $var_m_azp_gsc_request->sql_controller_table_up();
						# Instanciado funciones de la clase y asignando a variable
						$var_sql_shipping = $var_m_azp_gsc_request->sql_controller_shipping_number();
						#-------------------------------------------------------------------------#
						# Ruta principal de la carpeta
						$var_fl_route = 'systemfiles/filesproject/';
						# Ruta de la carpeta principal
						$var_fl_folder = 'azp_gsc_request/certificate/';
						# Ruta de la carpeta principal
						$var_fl_subfol = 'generated/';
						# Carpeta del año
						$var_fl_year = YEAR . '/';
						...
						$var_template_name = 'template_defunction.xlsx';
						...
						if (false != $var_sql_client) {
							...
							foreach ($var_sql_client as $key => $value) {
								...
								$var_password = $value[d_azp_gsc_client_t_password];
								# Informacion del banco o cliente de la base
								$var_remision_bank = $value[d_azp_gsc_client_t_name];
								# Informacion de la persona del banco o cliente de la base
								$var_remision_person = 'Atn: ' . $value[d_azp_gsc_client_t_person];
								# Informacion del cargo del banco o cliente de la base
								$var_remision_position = 'Cargo: ' . $value[d_azp_gsc_client_t_position];
								# Informacion de la ciudad del banco o cliente de la base
								$var_remision_city = 'Bogotá D.C.';
							}
						} else {
							# Informacion del banco o cliente de la base
							$var_password = '';
							# Informacion del banco o cliente de la base
							$var_remision_bank = 'BANCO DE BOGOTA';
							# Informacion de la persona del banco o cliente de la base
						...
						#-------------------------------------------------------------------------#
						# Validar si la variable es correcta y no falsa
						if (false != $var_sql_shipping) {
							# Recorrer datos de la base
							foreach ($var_sql_shipping as $key => $value) {
								# Informacion del banco o cliente de la base
								$var_shipping = $value['lbl_max'] + 1;
								//$var_shipping = $value['lbl_max'];
							}
						} else {
							# Informacion del banco o cliente de la base
							$var_shipping = 0;
						...
						#-------------------------------------------------------------------------#
						$var_text_certificate = 'Por medio de la presente, nos permitimos adjuntar certificado(s) de Defunción(s) según su solicitud correspondiente';
						#-------------------------------------------------------------------------#
						...
						$var_sheet->setCellValue('B16', $var_remision_city);
						$var_sheet->setCellValue('B20', $var_text_certificate);
						#-------------------------------------------------------------------------#
						...
						#-------------------------------------------------------------------------#
						$var_sheet->getStyle('B23:H23')->applyFromArray($var_style_header);
						# Validar si la variable es correcta y no falsa
						if (false != $var_sql_table) {
							# Establecer celda inicial
							$i = 24;
							...
							foreach ($var_sql_table as $key => $value) {
								...
								$var_sheet->getStyle('H' . $i)->applyFromArray($var_style_columns);
								# Asignar valores a las celdas de excel
								$var_sheet->setCellValue('B' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_identification']));
								$var_sheet->setCellValue('C' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_full_name']));
								$var_sheet->setCellValue('D' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_serial']));
								$var_sheet->setCellValue('E' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_city']));
								$var_sheet->setCellValue('F' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_number_request']));
								$var_sheet->setCellValue('G' . $i, html_entity_decode($value['lbl_azp_gsc_update_t_death_date']));
								$var_sheet->setCellValue('H' . $i, html_entity_decode($var_shipping));
								# Incrementar valores
								$i++;
							}
							if ($i < 35) {
								for ($j=$i; $j < 35; $j++) { 
									# Asignar valores a las celdas de excel
									$var_sheet->setCellValue('B' . $j, '');
									$var_sheet->setCellValue('C' . $j, '');
									$var_sheet->setCellValue('D' . $j, '');
									$var_sheet->setCellValue('E' . $j, '');
									$var_sheet->setCellValue('F' . $j, '');
									$var_sheet->setCellValue('G' . $j, '');
									$var_sheet->setCellValue('H' . $j, '');
								}
							}
						}
						#-------------------------------------------------------------------------#
						...
						$var_image_inicio->setCoordinates('G1');
					...
					case 'xlsxcompressone':
						...
						#-------------------------------------------------------------------------#
						# Nombre del archivo a guardar con sus datos respectivos
						$var_file_name = 'Sol_Proveedor_' . DATE . '_' . HOUR_NUMBER . '.xlsx';
						# Ruta de la carpeta para almacenar sus respectivos archivos
						$var_file_route = $var_fl_route . $var_fl_folder . $var_fl_subfol . $var_fl_year . $var_fl_month . $var_fl_date;
						# Ruta final concatenada para almacenar el archivo en su carpeta
						$var_file_final = '' . $var_file_route . $var_file_name;
						...
						exit();
						break;
					case 'xlsxcompressfour':
						#-------------------------------------------------------------------------#
						$var_em_id = $_GET['field_em_id'];
						$var_em_main = $_GET['field_em_main'];
						#-------------------------------------------------------------------------#
						# Instanciando clases del modelo
						$my_user = $var_m_azp_user->sql_select_tc_azp_user_data();
						# Recorrer datos de la base
						...
						#-------------------------------------------------------------------------#
						# Instanciado funciones de la clase y asignando a variable
						$var_sql_client = $var_m_azp_gsc_request->sql_controller_client($var_em_main);
						# Instanciado funciones de la clase y asignando a variable
						$var_sql_table = $var_m_azp_gsc_request->sql_controller_table();
						# Instanciado funciones de la clase y asignando a variable
						//$var_sql_table_up = $var_m_azp_gsc_request->sql_controller_table_up();
						...
						#-------------------------------------------------------------------------#
						# Nombre del archivo a guardar con sus datos respectivos
						$var_file_name = 'Remision_' . DATE . '_' . HOUR_NUMBER . '.xlsx';
						# Ruta de la carpeta para almacenar sus respectivos archivos
						$var_file_route = $var_fl_route . $var_fl_folder . $var_fl_subfol . $var_fl_year . $var_fl_month . $var_fl_date;
						# Ruta final concatenada para almacenar el archivo en su carpeta
						$var_file_final = '' . $var_file_route . $var_file_name;
						...
						#-------------------------------------------------------------------------#
						# Nombre del archivo a guardar con sus datos respectivos
						$var_encry_name = 'Remision_' . DATE . '_' . HOUR_NUMBER . '.xlsx';
						# Ruta de la carpeta para almacenar sus respectivos archivos
						$var_encry_route = $var_ec_route . $var_ec_folder . $var_ec_subfol . $var_ec_year . $var_ec_month . $var_ec_date;
						# Ruta final concatenada para almacenar el archivo en su carpeta
						$var_encry_final = '' . $var_encry_route . $var_encry_name;
						...
						exit();
						break;
					case 'xlsxidentification':
						# Ruta principal de la carpeta
						$var_ec_route = 'systemfiles/filesproject/';
						# Ruta de la carpeta principal
						$var_ec_folder = 'azp_gsc_request/images/';
						# Carpeta del año
						$var_ec_year = YEAR . '/';
						# Carpeta del año y mes
						$var_ec_month = YEAR_MONTH . '/';
						...
						# Consultar registros de la tabla por medio del modelo instanciado
						$var_sql = $var_m_azp_gsc_request->sql_azp_gsc_compressed_identification();
						# Verificiar si la sentencia query ejecutada es correcta y no falsa
						if (true == $var_sql) { $var_row = 2;
							# El constructor foreach proporciona un modo sencillo de iterar sobre arrays.
							foreach ($var_sql as $key => $value):
								# Asignar valores a las celdas de excel
								$var_sheet->setCellValue('A' . $var_row, html_entity_decode($value['lbl_azp_gsc_request_t_identification']));
								# Incrementar valores
								$var_row++;
							endforeach;
						}
					...
					case 'xlsxidentificationcl':
						...
						$var_sheet->setCellValue('C1', html_entity_decode('Serial'));
						$var_sheet->setCellValue('D1', html_entity_decode('Notaria'));
						$var_sheet->setCellValue('E1', html_entity_decode('Fecha Defuncion'));
						# Consultar registros de la tabla por medio del modelo instanciado
						$var_sql = $var_m_azp_gsc_request->sql_azp_gsc_compressed_identification_client();
						# Verificiar si la sentencia query ejecutada es correcta y no falsa
						if (true == $var_sql) { $var_row = 2;
							# El constructor foreach proporciona un modo sencillo de iterar sobre arrays.
							foreach ($var_sql as $key => $value):
								# Asignar valores a las celdas de excel
								$var_sheet->setCellValue('A' . $var_row, html_entity_decode($value['lbl_azp_gsc_request_t_identification']));
								$var_sheet->setCellValue('B' . $var_row, html_entity_decode($value['lbl_azp_gsc_request_t_full_name']));
								$var_sheet->setCellValue('C' . $var_row, html_entity_decode($value['lbl_azp_gsc_request_t_serial']));
								$var_sheet->setCellValue('D' . $var_row, html_entity_decode($value['lbl_azp_gsc_request_t_notary']));
								$var_sheet->setCellValue('E' . $var_row, html_entity_decode($value['lbl_azp_gsc_request_t_death_date']));
								# Incrementar valores
								$var_row++;
							endforeach;
						}
