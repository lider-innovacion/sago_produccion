		...
		*/
		final public static function pay(array $config, array $items, bool $individual = true, string $currency = 'USD') : array {
			...
			foreach ($items as $i => $item) {
				...
				${'item' . $i}->setName($item['item_name'])
					->setCurrency($currency)
					->setQuantity((int) $item['item_quantity'])
					->setPrice((float) $item['item_amount']);
				$cart[] = ${'item' . $i};
				$shipping += $individual ? $item['item_shipping'] : ($item['item_shipping'] * $item['item_quantity']);
				$tax += $individual ? $item['item_tax'] : ($item['item_tax'] * $item['item_quantity']);
				$subtotal += $item['item_amount'] * $item['item_quantity'];
			}
